...
| Count freeze policy | PRD §10/§12.6 enumerates HardFreeze, SoftFreeze, and NoFreeze; Architecture Overview and ERD state SoftFreeze-only v1. | Do not expose, hide, or enforce any policy option until the v1 contract is approved. | The conflict changes available controls and posting behavior. | `e01.2`, `eiams-frontend-e01.12` (D-WH-02 step 1 uses SoftFreeze), inventory-count module |
...
| D-OAS-02 | `contracts/openapi/eiams-v1.openapi.json` and `contracts/openapi/eiams-v1.provenance.json` | Historical admission record for the former architecture-owned provisional contract. D-INT-01 removes it from the active source-of-truth chain; the backend's real OpenAPI export now governs deterministic frontend generation and versioned production ratification. | `e01.6`, `e01.7`, `whhu`, `e08-t01`, `e08-t02`, all API-consuming tasks, release readiness |
| D-AUTH-01 | `docs/authentication-session-scope-contract-decision.md` | Defines memory-only bearer access tokens, rotating HttpOnly refresh cookies, authoritative session hydration, one required active scope and effective role, hierarchical authorization, `Selected | Unavailable` state, `401`/`403` behavior, and cache isolation after invalidation or authorized admin override. It exposes no ordinary-user scope selection. | `e01-t03`, `e01-t07`, `e08-t03`, `e06-t01`–`e06-t09`, `e07-t05`, `e22-t06`, `e24-t06`, `e26-t02` |
...
| D-RBAC-02 | `docs/route-permission-scope-matrix.md` (addendum) | Enterprise scope (المؤسسة) is governance-only: GOVERN oversight (monitor all sites/warehouses, dashboards, movement-following, statistics) and STRUCTURAL authority (org structure). NO inventory OPERATE permission and NO APPROVE gate in the transaction path. Permission axes (foundation): OPERATE + GOVERN in v1; APPROVE reserved for D-APP-01; ADMIN is structural and SYSTEM_ADMIN-only. Separates oversight from operation; corrects the loose "one role code, three scopes" reading so Enterprise = oversight, not a super-operator. Assignment cardinality unchanged (D-SRS-01). | `eiams-frontend-e01.9` (self), `e24-t06`, `e06-t06` |
| D-SRS-01 | `docs/single-role-single-scope-assignment-decision.md` | Every User has exactly one persistent role-scope assignment. Hierarchy expands effective authorization only; the session has a required active scope with no multi-scope selection. Backend/database/API enforcement remains a versioned `e01.7` ratification requirement. | `7ipk.1`–`7ipk.5`, `e01.7`, `whhu.1`, `whhu.7`, `whhu.10`, `whhu.11` |
...
| D-WH-01 | `docs/route-permission-scope-matrix.md` (D-WH-01 addendum) | Warehouse entity = separation-of-duties boundary, not a physical building; Interpretation A (multi-store at one location, each warehouse scoped + its own capability) vs Interpretation B (single warehouse with full capability matrix under one responsible manager). Single-warehouse + multiple domain-keepers is forbidden (governing rule); per-user material-domain restriction is out of scope for v1. | `eiams-frontend-e01.11`, `e10-t09`, `e24-t06`, `e01.12` (future D-WH-02) |
| D-APP-01 | `docs/adr/0003-digital-approval-and-signature-axis.md` | Forward-looking: APPROVE is a separate permission axis from OPERATE; the Enterprise director (GOVERN @ Enterprise) authorizes drafted transactions (Inter-branch transfers, adjustments, write-offs, high-value receipts) but never keys them in. Workflow `Draft -> PendingApproval -> Approved -> Posted` extends D-LIFE-01; signature evolution is authorization-click (audit-logged, phase 1) then PKI cryptographic signature (phase 2); anti-bottleneck via tiered/risk-based approval, delegation, and four-eyes dual control. v1-readiness only: reserve APPROVE-axis codes in the taxonomy, keep D-LIFE-01 extensible, rely on existing audit log; no v1 code, ratification of APPROVE codes and PKI deferred to the digital-signature phase. | `eiams-frontend-e01.10`, `e01.9` (D-RBAC-02 foundation), `e01-t04` (D-LIFE-01), `e01-t03` (D-AUTH-01) |
| D-WH-02 | `docs/adr/0002-warehouse-reorganization-is-document-driven.md` | Forward-looking: warehouse split is document-driven only (5-step pattern: SoftFreeze source, real Count, new warehouse.manage records, posted Transfer or e15 Opening Balance seeded from snapshot, then source set to Inactive — never delete). Pre-split documents stay immutable (e24-t08) and still reference the Inactive source; new warehouses only hold stock that a document put there; per-warehouse capability then prevents cross-domain mis-posting; user/role re-assignment flows through D-SRS-01. v1 already supports immutable ledgers, Active/Inactive status, SoftFreeze (BDM), Opening Balance (e15), and per-warehouse capability; gaps deferred: split-wizard UI, server-side Inactive rejection of postings, and Inactive-warehouse name resolution in reports. No v1 code. | `eiams-frontend-e01.12` (self), `eiams-frontend-e01.11` (D-WH-01 governing), `eiams-frontend-e15` (Opening Balance), `e24-t08` (immutable ledgers) |
...
- D-APP-01 is forward-looking only: v1 work must not implement APPROVE semantics or the PendingApproval state, but must keep the permission taxonomy and the D-LIFE-01 state machine extensible so the digital-signature phase can land without a v1 retrofit.
